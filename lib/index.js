...
export default function ({apikey:apikey,curl:curl}){
  const sso = (config={})=>onCreate({...config,...{name:'youloge.sso',target:'sso',apikey:apikey,curl:curl}})
  const pay = (config={})=>onCreate({...config,...{name:'youloge.pay',target:'pay',apikey:apikey,curl:curl}})
  const login = (config={})=>onCreate({...config,...{name:'youloge.login',target:'login',apikey:apikey,curl:curl}})
  const audio = (config={})=>onCreate({...config,...{name:'youloge.audio',target:'audio',apikey:apikey,curl:curl}})
  const video = (config={})=>onCreate({...config,...{name:'youloge.video',target:'video',apikey:apikey,curl:curl}})
  const payment = (config={})=>onCreate({...config,...{name:'youloge.payment',target:'payment',apikey:apikey,curl:curl}})
  const discuss = (config={})=>onCreate({...config,...{name:'youloge.discuss',target:'discuss',apikey:apikey,curl:curl}})
  const captcha = (config={})=>onCreate({...config,...{name:'youloge.captcha',target:'captcha',apikey:apikey,curl:curl}})
