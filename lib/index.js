...
const onCreate = (config)=>{
  ...
  // let URL = `http://localhost:5173/${target}.html`;
  ...
  const mask = selector && document.querySelector(selector) ? document.querySelector(selector) : document.createElement('div');
  if(selector && document.querySelector(selector)){
    for (const key in mask.dataset) {
      if (Object.hasOwnProperty.call(mask.dataset, key)) {
        config[key] = mask.dataset[key]
      }
    }
    mask.innerHTML = '';
    mask.appendChild(iframe);
  }else{
    mask.style = 'position: fixed; height: 100%; width: 100%; background: transparent; z-index: 2147483647; backdrop-filter: blur(2px);'
    mask.appendChild(iframe);
    document.body.appendChild(mask);
  }
  // 统一监听
  EventBus[hash] = {resolve:null,reject:null,event:null,mask:mask,iframe:iframe};
  const onMessage = ({origin,data,source})=>{
    let [keys] = Object.keys(data),{method,params} = data[keys] || {};
    let {resolve,reject,event,mask,iframe} = EventBus[keys];
    console.log(1,method,params,mask)
    if(method && URL.startsWith(origin) && keys === hash){
      let work = {
        'ready':()=>source.postMessage({[keys]:{method:'init',params:config}},origin),
        'success':()=>{(selector?iframe.remove():mask.remove());resolve && resolve.call(this,params)},
        'error':()=>{(selector?iframe.remove():mask.remove());reject && reject.call(this,params)}
      };
